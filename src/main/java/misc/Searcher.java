...
public class Searcher {
    ...
    public static <T extends Comparable<T>> IList<T> topKSort(int k, IList<T> input) {
        ...
        IPriorityQueue<T> arrayHeap = new ArrayHeap<T>();               
        if (k == 0) {
        	return result;
        	...
        	for(T value:input) {	
            	if (arrayHeap.size() < k) {
            		...
            		// the list has equal or greater value than the smallest element in heap
            		if (value.compareTo(arrayHeap.peekMin()) >= 0) { 
            			arrayHeap.removeMin();
            			arrayHeap.insert(value);
            		}
            	}
        ...
        while (!arrayHeap.isEmpty()) {
        	result.add(arrayHeap.removeMin());
        }
        return result;
    }
}
